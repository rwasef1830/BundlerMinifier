...
namespace BundlerMinifier
{
    ...
    {
        static readonly Regex s_RxUrl = new Regex(@"url\s*\(\s*([""']?)([^:<)]+)\1\s*\)", RegexOptions.IgnoreCase | RegexOptions.Compiled);
        ...
        public static string Adjust(string inputFile, string outputPath)
        {
            ...
            var matches = s_RxUrl.Matches(cssFileContents);
        ...
        static string GetAbsolutePath(string cssFilePath, string pathOnly)
        {
