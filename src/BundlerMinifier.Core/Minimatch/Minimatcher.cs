...
public class Options
{
    ...
    internal RegexOptions RegexOptions => this.NoCase ? RegexOptions.IgnoreCase : RegexOptions.None;
...
public class Minimatcher
{
    ...
    public static bool Check(string input, string pattern, Options options = null)
    {
        ...
        if (options is { NoComment: false } && pattern[0] == '#')
        {
            return false;
        }
    ...
    public Minimatcher(string pattern, Options options = null)
    {
        ...
        if (this._options.AllowWindowsPaths)
        {
    ...
    public bool IsMatch(string input) { return this.Match(input, false); }

    ///<summary>Filters a list of inputs against this pattern.</summary>
    public IEnumerable<string> Filter(IEnumerable<string> list)
    {
        list = list.Where(this.IsMatch);
        if (this._options.NoNull)
        {
            list = list.DefaultIfEmpty(this._pattern);
        }
    ...
    IEnumerable<string> _globSet;
    IEnumerable<IEnumerable<ParseItem>> _set;
    IEnumerable<IEnumerable<string>> _globParts;


    // any single thing other than /
    // don't need to escape / when using new RegExp()
    const string c_Qmark = "[^/]"
    ...
    static readonly HashSet<char> s_ReSpecials = new HashSet<char>("().*{}+?[]^$\\!".ToCharArray());
    static readonly Regex s_SlashSplit = new Regex("/+");

    void Make()
    {
        // empty patterns and comments match nothing.
        if (!this._options.NoComment && !string.IsNullOrEmpty(this._pattern) && this._pattern[0] == '#')
        {
            this._comment = true;
            return;
        }
        ...
        this._globSet = BraceExpand(this._pattern, this._options);
        ...
        this._globParts = this._globSet.Select(s => s_SlashSplit.Split(s)).ToList();

        // glob --> regexps
        this._set = this._globParts.Select(g => g.Select(t => this.Parse(t, false)))
    ...
    void ParseNegate()
    {
        ...
        if (this._options.NoNegate)
        {
            return;
        }

        for (var i = 0; i < this._pattern.Length && this._pattern[i] == '!'; i++)
        {
        ...
        if (negateOffset > 0)
        {
            this._pattern = this._pattern[negateOffset..];
        }
    }

    static readonly Regex s_HasBraces = new Regex(@"\{.*\}");
    static readonly Regex s_NumericSet = new Regex(@"^\{(-?[0-9]+)\.\.(-?[0-9]+)\}");
    // Brace expansion:
    // a{b,c}d -> abd acd
    // a{b,}c -> abc ac
    ...
    {
        if (options.NoBrace || !s_HasBraces.IsMatch(pattern))
        {
        ...
        if (pattern[0] != '{')
        {
            ...
            return BraceExpand(pattern[i..], options).Select(t => prefix + t);
        }
        ...
        var numset = s_NumericSet.Match(pattern);
        if (numset.Success)
        {
        ...
        return BraceExpand(pattern[i..], options).SelectMany(s1 => set.Select(s2 => s2 + s1));
    }

    [UsedImplicitly(ImplicitUseTargetFlags.WithMembers)]
    class PatternListEntry
    {
    ...
    class LiteralItem : ParseItem
    {
        public LiteralItem(string source) {
            this.Source = source; }
        public override string RegexSource(Options options) { return Regex.Escape(this.Source); }
        public override bool Match(string input, Options options)
        {
            return input.Equals(this.Source, options.NoCase ? StringComparison.OrdinalIgnoreCase : StringComparison.Ordinal);
        }
    }
    class MagicItem : ParseItem
    {
        ...
        {
            this.Source = source;
            this._regex = new Lazy<Regex>(() => new Regex("^" + source + "$", options.RegexOptions));
        }
        readonly Lazy<Regex> _regex;

        public override string RegexSource(Options options) { return this.Source; }
        public override bool Match(string input, Options options)
        {
            return this._regex.Value.IsMatch(input);
        }
    }
    class GlobStar : ParseItem
    {
        ...
        {
            return options.NoGlobStar ? c_Star
                : options.Dot ? c_TwoStarDot
                : c_TwoStarNoDot;
        }
    ...
    static readonly Regex s_EscapeCheck = new Regex(@"((?:\\{2})*)(\\?)\|");
    ...
    Tuple<ParseItem, bool> Parse(string pattern, bool isSub)
    {
        // shortcuts
        if (!this._options.NoGlobStar && pattern == "**")
        {
        ...
        string re = "";
        bool hasMagic = this._options.NoCase, escaping = false, inClass = false;
        ...
        for (var i = 0; i < pattern.Length; i++)
        {
            ...
            if (escaping && s_ReSpecials.Contains(c))
            {
                re += "\\" + c;
                escaping = false;
                continue;
            ...
            switch (c)
            {
                ...
                default:
                    ...
                    else if (s_ReSpecials.Contains(c) && !(c == '^' && inClass))
                    {
                        re += "\\";
                    }
        ...
        if (inClass)
        {
            ...
            string cs = pattern[(classStart + 1)..];
        ...
        while (patternListStack.Any())
        {
            ...
            tail = s_EscapeCheck.Replace(tail, m =>
            {
        ...
        return new Tuple<ParseItem, bool>(new MagicItem(re, this._options), false);
    }


    Regex MakeRegex()
    {
        if (this._regexp != null || this._isError)
        {
            return this._regexp;
        }
        ...
        if (this._comment || this._empty || !this._set.Any())
        {
            this._isError = true;
            return null;
        }
        var re = string.Join("|", this._set.Select(p =>
            string.Join("\\/", p.Select(p2 => p2.RegexSource(this._options))
            )));
        ...
        try
        {
            return this._regexp = new Regex(re, this._options.RegexOptions);
        }
        catch
        {
            this._isError = true;
    ...
    bool Match(string input, bool partial)
    {
        ...
        var f = s_SlashSplit.Split(input);
        //if (options.debug) {
        ...
        foreach (var localPattern in this._set)
        {
            var hit = this.MatchOne(f, localPattern.ToList(), partial);
            if (!hit)
            {
                continue;
            }

            if (this._options.FlipNegate)
            {
                return true;
            }
    ...
    // the parts match.
    bool MatchOne(IList<string> file, IList<ParseItem> localPattern, bool partial)
    {
        ...
        if (this._options.MatchBase && localPattern.Count == 1)
        {
        ...
        //}
        int fi = 0, pi = 0;
        for (; fi < file.Count && pi < localPattern.Count; fi++, pi++)
        {

            //if (options.debug) {
            //  console.error("matchOne loop")
            //}
            var p = localPattern[pi];
            string f = file[fi];
            ...
            switch (p)
            {
                ...
                case GlobStar _:
                {
                    ...
                    if (pr == localPattern.Count)
                    {
                        ...
                        {
                            if (file[fi] == "." || file[fi] == ".." ||
                                (!this._options.Dot && !string.IsNullOrEmpty(file[fi]) && file[fi][0] == '.'))
                            {
                                return false;
                            }
                        }
                    ...
                    while (fr < file.Count)
                    {
                        ...
                        if (this.MatchOne(file.Skip(fr).ToList(), localPattern.Skip(pr).ToList(), partial))
                        {
                            //if (options.debug)
        ...
        if (fi == file.Count && pi == localPattern.Count)
        {
    ...
    static readonly Regex s_GlobUnescaper = new Regex(@"\\(.)");
        
    static string GlobUnescape(string s)
    {
        return s_GlobUnescaper.Replace(s, "$1");
    }
}